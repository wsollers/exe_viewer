...
- `known-win-x86.exe` — deterministic minimal PE32 executable fixture with `.text`,
  ...
  directory, one TLS directory with one callback, one overlay certificate table
  entry, one Load Config directory with Guard CF fields, and `IMAGE_FILE_MACHINE_I386`.
- `known-win-x64.exe` — deterministic minimal PE32+ executable fixture with a
  ...
  CodeView RSDS debug record, one runtime-function entry in the exception
  directory, one `.tls` section with one TLS directory/callback, one overlay
  certificate table entry, one Load Config directory with Guard CF fields, and
