...
Result<FileInfo> parse_elf_bytes(std::span<const std::uint8_t> bytes) {
    if (bytes.size() < 0x20) {
        return make_error("ELF file too small");
    }
    ...
    if (ei_data != 1) {
        return make_error("ELF big-endian not supported yet");
    }
