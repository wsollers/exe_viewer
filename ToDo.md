...
>
> **Progress (Phase 1):** Phase 0 complete (P0-1 … P0-10). Phase 1 in progress — P1-1 / P1-2 done; P1-3 / P1-4 implemented in the **core** (`PeImage` / `ElfImage` identity behind `IBinaryImage`, via `parse_image`), with the app-side migration (BinaryModel/panels onto `IBinaryImage`, removing the legacy parsers) still pending. Unit suite: 41 tests. The app load path now calls `parse_image()` (ELF files load; PE panels still on the legacy `PeModel`); full panel migration + legacy-parser removal pending. Phase 2 started: section tables parsed for PE/ELF (P2-1), ELF program-header segments parsed and shown in the Sections panel, and `IBinaryImage` now exposes checked file-offset ↔ virtual-address mapping. Phase 3 started: ELF `.symtab` / `.dynsym` symbols parse into shared `Symbol` values and display in a Symbols panel; ELF `DT_NEEDED` libraries and PE import directories parse into shared `ImportEntry` values and display in a unified Imports panel; PE export directories parse into shared `ExportEntry` values and display in a unified Exports panel. Deterministic parser fixtures now cover PE32/PE32+, PE base relocations, ELF32/ELF64, little-/big-endian ELF identity, ARM32/ARM64, RISC-V32/RISC-V64, MIPS32/MIPS64, PowerPC32/PowerPC64, `.text` mapping, raw ELF `.symtab` records, raw ELF `.dynamic` records, raw ELF `.rel.dyn` / `.rela.dyn` relocation records, `PT_INTERP` interpreter paths, ELF notes from both `PT_NOTE` and `SHT_NOTE`, and SysV/GNU ELF hash tables. ELF file-header, program-header, section-header, symbol-table, dynamic-entry, relocation, interpreter, note, and hash-table parsing now exposes every raw field or resolved value currently modeled and is tested across the fixture matrix; PE base-relocation blocks are now exposed and tested for PE32/PE32+. Disassembler is now initialized from the unified image (PE **and** supported ELF architectures), ELF entry-point disassembly resolves through core VA→file-offset mapping, clicking a byte in the Hex view disassembles a window using the mapped VA when available (P4-1 partial), unsupported architectures no longer fall back to bogus x64 disassembly, and Capstone mode setup is covered by tests for x86-64, ARM64, MIPS32/MIPS64, PowerPC32/PowerPC64, RISC-V32, and RISC-V64; also fixed an ELF crash in `on_file_loaded` (unconditional null-`pe()` deref, exposed once ELF files began loading).
...
- [~] **P3-6** **PE full-spec metadata backlog:** base relocations now parse into raw `PeBaseRelocationBlock` / `PeBaseRelocationEntry` values exposed via `IBinaryImage::pe_base_relocations()` and are covered by deterministic PE32/PE32+ fixtures. Remaining: resources, TLS directory, load config / CFG / security metadata, exception/unwind data (`.pdata` / `.xdata`), debug directory / CodeView records, certificate table, bound imports, CLR/COM descriptor, section COMDAT/COFF symbol data, rich header (optional), overlay detection, ordinal-import coverage, forwarder-export coverage, delay-load imports, and malformed-table tests. Add PE DLL fixture coverage as these land.
