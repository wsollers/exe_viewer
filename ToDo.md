...
- [~] **P3-5** **ELF full-spec metadata backlog:** relocations (`.rel*` / `.rela*`) and interpreter path (`PT_INTERP`) now have parser/model/test coverage across the deterministic fixture matrix. Remaining: PLT/GOT mapping, additional dynamic tags beyond `DT_NEEDED`, symbol versioning (`.gnu.version*`), GNU hash / SysV hash tables, notes (`PT_NOTE`, `.note.*`), GNU properties, TLS, init/fini arrays, compressed sections, section groups, and OS/processor-specific flags. Keep each feature behind shared value types where possible and add targeted malformed-table tests.
