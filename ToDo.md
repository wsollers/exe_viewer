...
- [~] **P3-4** Map all of the above onto shared `ImportEntry`/`ExportEntry`/`Symbol` types so the panels are format-agnostic, with a small "format-specific extras" expander. **Progress:** shared `Symbol` / `ImportEntry` / `ExportEntry` types, Symbols panel, unified Imports panel, and unified Exports panel exist. Remaining: PE import migration and format-specific extras.
