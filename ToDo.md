...
- [ ] **P6-4** **Default docking layout and reset:** replace the current "all windows appear on top of each other" startup behavior with a deterministic ImGui docking workspace. Default: menu/toolbar at top, status/log strip at bottom, navigation tree docked left, primary detail/hex/disassembly area in the center, and inspector/properties docked right. Allow all panes to be torn off/moved, persist user layout, and provide **View → Reset Layout** to restore the known default.
