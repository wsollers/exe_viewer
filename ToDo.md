...
- [~] **P3-6** **PE full-spec metadata backlog:** base relocations now parse into raw `PeBaseRelocationBlock` / `PeBaseRelocationEntry` values exposed via `IBinaryImage::pe_base_relocations()`, and debug directories now parse into raw `PeDebugDirectory` values with CodeView RSDS GUID/age/PDB path extraction. Both are covered by deterministic PE32/PE32+ fixtures. Remaining: resources, TLS directory, load config / CFG / security metadata, exception/unwind data (`.pdata` / `.xdata`), certificate table, bound imports, CLR/COM descriptor, section COMDAT/COFF symbol data, rich header (optional), overlay detection, ordinal-import coverage, forwarder-export coverage, delay-load imports, and malformed-table tests. Add PE DLL fixture coverage as these land.
