...
- [ ] **P6-9** **XPEViewer-inspired structure navigator:** add a stable left-side tree that represents the loaded image hierarchy, inspired by XPEViewer's structure-first workflow: File → headers → directories → sections/segments → imports/exports/symbols/relocations/debug/resources/TLS/etc. Tree nodes should be format-aware but consistent across PE/ELF where possible. Single-click selects a node and updates the center/right panes; double-click or Enter follows the node to the best byte/disassembly location when one exists.
- [ ] **P6-10** **Selection and navigation model:** introduce a viewer-model selection state independent of ImGui widgets. A selected item should carry kind, display label, file offset, RVA/VA, size, backing parser object id/index, and preferred view (`details`, `hex`, `disassembly`). This enables tree→details, section→hex, symbol/import/export→disassembly, and future live-memory navigation without ad hoc cross-panel calls.
