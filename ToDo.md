...
- [~] **P6-4** **Default docking layout and reset:** first slice implemented with ImGui DockBuilder. On first run, the viewer builds a deterministic dock workspace: File/Sections/Symbols on the left, Hex in the primary center area, Disassembly below the center, Imports/Exports/PE-specific detail panes on the right, and Log at the bottom. **View → Reset Layout** rebuilds the default while panes remain dockable/tearable. Remaining: add the dedicated structure tree and inspector panes from P6-9/P6-11, decide persisted-layout migration/versioning for users with older broken `imgui.ini` state, and add recent-files/About dialog.
...
- [~] **P6-9** **XPEViewer-inspired structure navigator:** first model slice added in `navigation/viewer_navigation.*`: `build_structure_tree()` creates a stable hierarchy from `IBinaryImage` for File/Image → headers → sections → segments → imports/exports/symbols → PE relocations/debug directories → ELF interpreter/dynamic entries/relocations/notes/hash tables. Fixture-backed tests cover PE and ELF tree contents. Remaining: render this as a left-side ImGui tree, wire click/double-click/Enter behavior, and add context menus/details pane integration.
- [~] **P6-10** **Selection and navigation model:** first ImGui-independent selection payload added via `ViewerSelection`: kind, display label, optional file offset, optional VA, size, backing object index, and preferred view (`details`, `hex`, `disassembly`). Tests verify executable sections prefer disassembly and parser objects expose expected locations. Remaining: centralize live selection state in `UiApp`/viewer model and have panels consume it instead of owning ad hoc callbacks.
- [ ] **P6-11** **Details and inspector panes:** standardize interaction: left-click selects and fills the center details view; right-click opens a context menu with actions such as "Show details", "Go to offset", "Go to RVA/VA", "Open in hex", "Disassemble here", "Copy value", and "Copy row as JSON". Reserve the right pane for compact inspector/properties for the current selection, not another competing primary table.
