...
- [~] **P3-4** Map all of the above onto shared `ImportEntry`/`ExportEntry`/`Symbol` types so the panels are format-agnostic, with a small "format-specific extras" expander. **Progress:** shared `Symbol` / `ImportEntry` types, Symbols panel, and unified Imports panel exist. Remaining: shared export value type, PE import/export migration, and format-specific extras.
