...
>
> **Progress (Phase 1):** Phase 0 complete (P0-1 … P0-10). Phase 1 in progress — P1-1 / P1-2 done; P1-3 / P1-4 implemented in the **core** (`PeImage` / `ElfImage` identity behind `IBinaryImage`, via `parse_image`), with the app-side migration (BinaryModel/panels onto `IBinaryImage`, removing the legacy parsers) still pending. Unit suite: 44 tests. The app load path now calls `parse_image()` (ELF files load; PE panels still on the legacy `PeModel`); full panel migration + legacy-parser removal pending. Phase 2 started: section tables parsed for PE/ELF (P2-1), ELF program-header segments parsed and shown in the Sections panel, and `IBinaryImage` now exposes checked file-offset ↔ virtual-address mapping. Phase 3 started: ELF `.symtab` / `.dynsym` symbols parse into shared `Symbol` values and display in a Symbols panel; ELF `DT_NEEDED` libraries and PE import directories parse into shared `ImportEntry` values and display in a unified Imports panel; PE export directories parse into shared `ExportEntry` values and display in a unified Exports panel. Deterministic parser fixtures now cover PE32/PE32+, PE base relocations, PE debug directory / CodeView records, ELF32/ELF64, little-/big-endian ELF identity, ARM32/ARM64, RISC-V32/RISC-V64, MIPS32/MIPS64, PowerPC32/PowerPC64, `.text` mapping, raw ELF `.symtab` records, raw ELF `.dynamic` records, raw ELF `.rel.dyn` / `.rela.dyn` relocation records, `PT_INTERP` interpreter paths, ELF notes from both `PT_NOTE` and `SHT_NOTE`, and SysV/GNU ELF hash tables. ELF file-header, program-header, section-header, symbol-table, dynamic-entry, relocation, interpreter, note, and hash-table parsing now exposes every raw field or resolved value currently modeled and is tested across the fixture matrix; PE base-relocation blocks and CodeView debug records are now exposed and tested for PE32/PE32+. Disassembler is now initialized from the unified image (PE **and** supported ELF architectures), ELF entry-point disassembly resolves through core VA→file-offset mapping, clicking a byte in the Hex view disassembles a window using the mapped VA when available (P4-1 partial), unsupported architectures no longer fall back to bogus x64 disassembly, and Capstone mode setup is covered by tests for x86-64, ARM64, MIPS32/MIPS64, PowerPC32/PowerPC64, RISC-V32, and RISC-V64; also fixed an ELF crash in `on_file_loaded` (unconditional null-`pe()` deref, exposed once ELF files began loading). The first UI navigation-model slice is in place: a tested, ImGui-independent structure tree builds selectable nodes from `IBinaryImage` for PE and ELF.
...
- [~] **P2-1** Section **table** populated for both formats in `binary_image.cpp`, exposed via `IBinaryImage::sections()`: name (PE inline 8-char; ELF resolved via `.shstrtab`), virtual address (PE = ImageBase+RVA; ELF `sh_addr`), virtual size, file offset/size (ELF `SHT_NOBITS`→0), and R/W/X (PE `IMAGE_SCN_MEM_*`; ELF `SHF_ALLOC`/`WRITE`/`EXECINSTR`), all bounds-checked. ELF program headers are exposed as raw `ElfProgramHeader` values via `IBinaryImage::elf_program_headers()` and projected to shared `Segment` values. ELF section headers are exposed as raw `ElfSectionHeader` values via `IBinaryImage::elf_section_headers()` and projected to shared `Section` values. Tested against the deterministic ELF endian/class/architecture matrix and PE32+ x64 fixture. App: the Sections panel shows sections and ELF segments from `IBinaryImage`. **Remaining:** section↔segment mapping; click-through to hex — P2-2/P2-3.
...
- [~] **P3-6** **PE full-spec metadata backlog:** base relocations now parse into raw `PeBaseRelocationBlock` / `PeBaseRelocationEntry` values exposed via `IBinaryImage::pe_base_relocations()`, debug directories parse into raw `PeDebugDirectory` values with CodeView RSDS GUID/age/PDB path extraction, and TLS directories parse into raw `PeTlsDirectory` values with PE32/PE32+ fixed fields plus callback VA arrays. These are covered by deterministic PE32/PE32+ fixtures. Remaining: resources, load config / CFG / security metadata, exception/unwind data (`.pdata` / `.xdata`), certificate table, bound imports, CLR/COM descriptor, section COMDAT/COFF symbol data, rich header (optional), overlay detection, ordinal-import coverage, forwarder-export coverage, delay-load imports, and malformed-table tests. Add PE DLL fixture coverage as these land.
...
- [~] **P6-9** **XPEViewer-inspired structure navigator:** first model slice added in `navigation/viewer_navigation.*`: `build_structure_tree()` creates a stable hierarchy from `IBinaryImage` for File/Image → headers → sections → segments → imports/exports/symbols → PE relocations/debug directories → ELF interpreter/dynamic entries/relocations/notes/hash tables. Fixture-backed tests cover PE and ELF tree contents. First UI slice added a left-side `Structure` panel that renders the tree and updates the current selection on single-click. Remaining: double-click/Enter navigation, context menus, richer node labels/icons, and tree filtering.
- [~] **P6-10** **Selection and navigation model:** first ImGui-independent selection payload added via `ViewerSelection`: kind, display label, optional file offset, optional VA, size, backing object index, and preferred view (`details`, `hex`, `disassembly`). Tests verify executable sections prefer disassembly and parser objects expose expected locations. `UiApp` now owns a current selection and rebuilds the structure tree on file load. Remaining: have hex/disassembly/details actions consume selection directly instead of owning ad hoc callbacks, then test behavior through a viewer-model layer.
- [~] **P6-11** **Details and inspector panes:** first `Details` pane added on the right. Single-clicking a structure node fills it with kind, label, preferred view, file offset, VA, size, and backing object index. Remaining: right-click context menu actions ("Show details", "Go to offset", "Go to RVA/VA", "Open in hex", "Disassemble here", "Copy value", "Copy row as JSON") and richer parser-specific property tables.
